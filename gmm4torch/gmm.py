...
class GaussianMixture(torch.nn.Module):
    ...
    Input tensors are expected to be flat with dimensions (n: number of
    samples, d: number of features).

    The model then extends them to (n, 1, d).

    The model parametrization (mu, sigma) is stored as (1, k, d), probabilities
    are shaped (n, k, 1) if they relate to an individual sample, or (1, k, 1)
    ...
    def bic(self, x):
        ...
        free_params = (self.n_features * self.n_components + self.n_features +
                       self.n_components - 1)

        bic = -2. * self.__score(
            x, as_average=False).mean() * n + free_params * np.log(n)
    ...
    def fit(self, x, delta=1e-3, n_iter=100, warm_start=False):
        ...
        while (i <= n_iter) and (j >= delta):
            ...
            if torch.isinf(self.log_likelihood.abs()) or torch.isnan(
                ...
                # When the log-likelihood assumes unbound values, reinitialize
                # model
                ...
                if self.init_params == "kmeans":
                    self.mu.data, = self.get_kmeans_mu(
                        x, n_centers=self.n_components)
    ...
    def _e_step(self, x):
        ...
        Also returns the mean of the mean of the logarithms of the
        probabilities (as is done in sklearn).
    ...
    def _m_step(self, x, log_resp):
        ...
        if self.covariance_type == "full":
            ...
            var = torch.sum((x - mu).unsqueeze(-1).matmul(
                (x - mu).unsqueeze(-2)) * resp.unsqueeze(-1),
                            dim=0,
    ...
    def __update_mu(self, mu):
        ...
        assert mu.size() in [(self.n_components, self.n_features),
                             (1, self.n_components, self.n_features)
                             ], "Input mu does not have required tensor "
        "dimensions (%i, %i) or (1, %i, %i)" % (
    ...
    def get_kmeans_mu(self, x, n_centers, init_times=50, min_delta=1e-3):
        """
        Find an initial value for the mean. Requires a threshold min_delta for
        the k-means algorithm to stop iterating. The algorithm is repeated
        init_times often, after which the best centerpoint is returned.
        ...
        for i in range(init_times):
            ...
            l2_dis = torch.norm(
                (x.unsqueeze(1).repeat(1, n_centers, 1) - tmp_center),
                p=2,
                dim=2)
